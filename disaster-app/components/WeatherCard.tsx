...
const getWeatherIcon = (weatherType: string): keyof typeof Ionicons.glyphMap => {
  ...
  if (type.includes("partly cloudy")) return "cloudy-outline";
  if (type.includes("cloud")) return "cloud";
  if (type.includes("fog")) return "cloudy-night-outline";
  if (type.includes("drizzle")) return "rainy-outline";
  ...
  if (type.includes("wind")) return "wind-outline";        // valid icon
  ...
  return (
    ...
    >
      <View style={{ flexDirection: "column", alignItems: "center" }}>
        ...
        <Text style={{ color: "#E5E5E5", marginTop: 5, fontSize: 12 }}>
         <Feather name="wind"/> {weather.wind}Kmph |  <Ionicons name="water" style={{color:"#E5E5E5"}}/> {weather.humidity}%
        </Text>

        <Text style={{ color: "#FFFFFF", fontSize: 12, fontWeight: "bold" }}>
          <FontAwesome6 name="location-dot" size={10} color="#FFFFFF" />
          {location?.district}
        </Text>
      </View>
